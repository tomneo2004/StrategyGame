...
public class WoodResourceGenerator : MonoBehaviour, IArchitecture 
{
	...
	void DoFetchData()
	{
		...
		SawmillBuildingMetaData data = SawmillBuildingMetaData.Load (true);
		...
		if((data.hasTask) && (_task == null))
		{
			...
			_task = WoodResourceGenerateTask.CreateTask<WoodResourceGenerateTask>(data.resourceId, data.resourceRegenPerDuration, OnResourceTaskComplete, OnTimeLeftToComplete, data.taskDuration);
	...
	*/
	...
	void AddResourceRegenTask()
	{
		...
		*/
		...
		if(DoStartRegenTask())
		{
			_task = WoodResourceGenerateTask.CreateTask<WoodResourceGenerateTask>(resourceId, resourceRegenPerDuration, OnResourceTaskComplete, OnTimeLeftToComplete, collectPerDuration);
